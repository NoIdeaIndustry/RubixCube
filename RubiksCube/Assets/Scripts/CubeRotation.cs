...
public class CubeRotation : MonoBehaviour
{
    ...
    private IEnumerator RotateSubCube(GameObject subCube, Vector3 axis, bool reverse, float speed)
    {
        ...
        Quaternion targetRot = subCube.transform.localRotation * MathExtensions.QuaternionAxisAngle(localAxis, reverse ? -90 : 90);

        while (subCube.transform.localRotation != targetRot)
        {
            // Smoothly interpolate between the sub-cube's start rotation and its target rotation.
            float remainingAngle            = MathExtensions.QuaternionAngle(subCube.transform.localRotation, targetRot);
            float rotationAmount            = Mathf.Min(1f, (speed * Time.deltaTime) / remainingAngle);
            subCube.transform.localRotation = MathExtensions.QuaternionSlerp(subCube.transform.localRotation, targetRot, rotationAmount);

            // Smoothly rotate the sub-cube's position around the rotation normal.
            Quaternion gradualRot           = MathExtensions.QuaternionAxisAngle(axis, speed * Time.deltaTime * (reverse ? -1 : 1));
        ...
        subCube.transform.localPosition    = MathExtensions.QuaternionAxisAngle(axis, reverse ? -90 : 90) * startPos;
